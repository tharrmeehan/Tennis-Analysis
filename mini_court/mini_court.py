...
class MiniCourt:
    ...
    def convert_meters_to_pixels(self, meters):
        return convert_meters_to_pixel_distance(
            meters, constants.DOUBLE_LINE_WIDTH, self.court_drawing_rectangle_width
        )
    ...
    def set_court_drawing_keypoints(self):
        ...
        drawing_key_points[6] = (
            drawing_key_points[0] + self.court_drawing_rectangle_width
        )
    ...
    def set_mini_court_position(self):
        ...
        self.court_drawing_rectangle_width = self.court_end_x - self.court_start_x
    ...
    def draw_court(self, frame):
        ...
        net_end = (
            ...
            int(self.drawing_key_points[3] + self.drawing_key_points[7]) // 2,
        )
    ...
    def get_width_of_mini_court(self):
        return self.court_drawing_rectangle_width
    ...
    def get_minicourt_coordinates(
        ...
        distance_from_keypoint_x_pixels, distance_from_keypoint_y_pixels = (
            measure_xy_distance(obj_pos, closest_keypoint)
        )
        ...
        # Convert mini court coordinates to real court coordinates
    ...
    def convert_bounding_boxes_to_mini_court_coords(
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            ball_position = get_center_position_bbox(ball_box)
            closest_player_id_to_ball = min(
                player_bbox.keys(),
                key=lambda x: measure_distance(
                    ball_position, get_center_position_bbox(player_bbox[x])
                ),
            )
            ...
            for player_id, bbox in player_bbox.items():
                ...
                bboxes_height_in_pixels = [
                    get_height_of_bbox(player_boxes[i][player_id])
                    for i in range(frame_index_min, frame_index_max)
                ]
                max_player_height = max(bboxes_height_in_pixels)

                mini_court_player_position = self.get_minicourt_coordinates(
                    foot_position,
                    closest_keypoint,
    ...
    def draw_points_on_minicourt(self, frames, positions, color=(0, 0, 255)):
